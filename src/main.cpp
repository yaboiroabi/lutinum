...
#if defined(NDEBUG)
# error "Lutinum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x84761f0c5740e8e517aaabcd42541202bba9816a4469832622035e683fbca41b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lutinum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lutinum Signed Message:\n";
...
{
    // Lutinum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000000); // Lutinum: 84000k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Lutinum: 5 days
static const int64 nTargetSpacing = 1 * 60; // Lutinum: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lutinum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lutinum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfe475cee63e82d0bcba6da4070b61b5c8c33c4fc74a7c4f942a24f56ae0a8e9a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Trekmovie.com 25/Nov/2021 Review - Star Trek - Discovery confronts uncertainty in Anomaly";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0436bb97dff62204ceb0bcd743282ee91a95f165c8ebaeb025551509c2c7df76976abafffdf133bf803d633e09db22125490de7c0554f63fe1d92a683d2e0f272d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637894395;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087204384;

        if (fTestNet)
        {
            block.nTime    = 1637894358;
            block.nNonce   = 391574541;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd8d50cc89988def181890785f867753ade2da93202a6e32b0735bae3478020cf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xbd, 0xd4 }; // Lutinum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LutinumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LutinumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LutinumMiner(CWallet *pwallet)
{
    printf("LutinumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lutinum-miner");
    ...
    try { loop {
        ...
        printf("Running LutinumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LutinumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LutinumMiner, pwallet));
}
